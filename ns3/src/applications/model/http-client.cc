...
std::string
HttpClientApplication::GetRemoteAddress ()
{
  std::stringstream _addr;
  m_publicPeerAddress.Print (_addr);
  return _addr.str ();
}
...
HttpClientApplication::SetRemote (Ipv4Address ip, uint16_t port)
{
  ...
  m_publicPeerAddress = ip;
  m_peerPort = port;
}

void
HttpClientApplication::SetRemote (Ipv6Address ip, uint16_t port)
{
  NS_LOG_FUNCTION (this << ip << port);
  m_peerAddress = Address (ip);
  m_peerPort = port;
...
void
HttpClientApplication::LogStateChange(const ns3::TcpSocket::TcpStates_t old_state, const ns3::TcpSocket::TcpStates_t new_state)
{
...
HttpClientApplication::TryEstablishConnection (void)
{
  ...
  if (!m_keepAlive || m_socket == 0)
  {
    ...
    if (m_socket == 0)
    {
      ...
      m_socket->SetFlowType("Long");
      m_socket->SetOutputFileName("NULL");
      if (Ipv4Address::IsMatchingType(m_peerAddress) == true)
      {
        m_socket->Bind();
        int errno = m_socket->Connect (InetSocketAddress (Ipv4Address::ConvertFrom(m_peerAddress), m_peerPort));
        m_socket->SetFlowId(0);
        m_socket->SetDupAckThresh(0);
...
void
HttpClientApplication::StartApplication (void)
{
  NS_LOG_FUNCTION (this);
...
HttpClientApplication::ConnectionClosedError (Ptr<Socket> socket)
{
 ///fprintf(stderr,"Client(%d): Socket was closed with an error, errno=%d; Trying to open it again...\n", node_id, socket->GetErrno());
  ...
  if(m_keepAlive) Simulator::Schedule(Seconds(0.5), &HttpClientApplication::TryEstablishConnection, this);
}
...
void
HttpClientApplication::StopApplication ()
{
  NS_LOG_FUNCTION (this);
...
void
HttpClientApplication::ConnectionComplete (Ptr<Socket> socket)
{
...
void
HttpClientApplication::DoSendGetRequest (Ptr<Socket> s, uint32_t txSpace)
{
  NS_LOG_FUNCTION (this);

  Ptr<MpTcpSocketBase> localSocket = DynamicCast<MpTcpSocketBase> (s);
...
void
HttpClientApplication::CancelDownload()
{
...
uint32_t
HttpClientApplication::ParseResponseHeader(const uint8_t* buffer, size_t len, int* realStatusCode, unsigned int* contentLength)
{
/*
  HTTP/1.1 200 OKCRLF
  ...
  */
  ...
  NS_LOG_DEBUG ("Client("<<node_id<<"): Parsing Response Header of length " << len);
...
void
HttpClientApplication::OnFileReceived(unsigned status, unsigned length)
{
  if (!m_active)
    return;
...
void
HttpClientApplication::HandleRead (Ptr<Socket> s)
{
  ...
  while ((packet = socket->Recv ()) && (packet->GetSize() != 0)) // Vitalii: before mptcp it was while (true)
  {
