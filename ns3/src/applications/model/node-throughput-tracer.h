...
#ifndef NODE_THROUGHPUT_TRACER
#define NODE_THROUGHPUT_TRACER
 ...
 */
class NodeThroughputTracer : public ns3::Object {
public:
  static void
  Destroy();
   ...
   *          for the lifetime of simulation, otherwise SEGFAULTs are inevitable
   */
  static Ptr<NodeThroughputTracer>
   ...
   */
  NodeThroughputTracer(boost::shared_ptr<std::ofstream> os, Ptr<Node> node);
   ...
   */
  NodeThroughputTracer(boost::shared_ptr<std::ofstream> os, const std::string& node);

  /**
   * @brief Destructor
   */
  ~NodeThroughputTracer();
...
private:
  ...
  void
  ThroughputStats(Ptr<ns3::Application> app, uint64_t txBytes, uint64_t rxBytes, uint32_t connectionCount);
