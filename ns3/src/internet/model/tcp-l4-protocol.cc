...
TcpL4Protocol::GetTypeId(void)
{
  static TypeId tid = TypeId("ns3::TcpL4Protocol")
      .SetParent<IpL4Protocol>().AddConstructor<TcpL4Protocol>()
      .AddAttribute("RttEstimatorType",
          ...
          MakeTypeIdChecker())
      .AddAttribute("SocketType","Socket type of TCP objects.",
...
Ipv4EndPoint *
TcpL4Protocol::Allocate(Ipv4Address localAddress, uint16_t localPort, Ipv4Address peerAddress, uint16_t peerPort)
{
...
Ipv6EndPoint *
TcpL4Protocol::Allocate6(Ipv6Address localAddress, uint16_t localPort, Ipv6Address peerAddress, uint16_t peerPort)
{
  NS_LOG_FUNCTION (this << localAddress << localPort << peerAddress << peerPort);
  return m_endPoints6->Allocate(localAddress, localPort, peerAddress, peerPort);
}
...
void
TcpL4Protocol::ReceiveIcmp(Ipv4Address icmpSource, uint8_t icmpTtl, uint8_t icmpType, uint8_t icmpCode, uint32_t icmpInfo,
    Ipv4Address payloadSource, Ipv4Address payloadDestination, const uint8_t payload[8])
{
...
void
TcpL4Protocol::ReceiveIcmp(Ipv6Address icmpSource, uint8_t icmpTtl, uint8_t icmpType, uint8_t icmpCode, uint32_t icmpInfo,
    Ipv6Address payloadSource, Ipv6Address payloadDestination, const uint8_t payload[8])
{
  NS_LOG_FUNCTION (this << icmpSource << icmpTtl << icmpType << icmpCode << icmpInfo
      << payloadSource << payloadDestination);
  uint16_t src, dst;
...
TcpL4Protocol::Receive(Ptr<Packet> packet, Ipv4Header const &ipHeader, Ptr<Ipv4Interface> incomingInterface)
{
      ...
      if ((opt->optName == OPT_MPC) && hasSyn)
        ...
        { // In this case there should be endPoint with this token, so look for a match on all endpoints.
          ...
          if (it != m_TokenMap.end())
            {
              NS_LOG_INFO("TcpL4Protocol::Receive -> OPT_JOIN -> Token " << Token << " has find forwardup to it");
...
enum IpL4Protocol::RxStatus
TcpL4Protocol::Receive(Ptr<Packet> packet, Ipv6Header const &ipHeader, Ptr<Ipv6Interface> interface)
{
  ...
  NS_LOG_LOGIC ("TcpL4Protocol "<<this<<" received a packet");
  Ipv6EndPointDemux::EndPoints endPoints = m_endPoints6->Lookup(ipHeader.GetDestinationAddress(), tcpHeader.GetDestinationPort(),
...
void
TcpL4Protocol::Send(Ptr<Packet> packet, Ipv4Address saddr, Ipv4Address daddr, uint16_t sport, uint16_t dport, Ptr<NetDevice> oif)
{
...
void
TcpL4Protocol::Send(Ptr<Packet> packet, Ipv6Address saddr, Ipv6Address daddr, uint16_t sport, uint16_t dport, Ptr<NetDevice> oif)
{
  NS_LOG_FUNCTION (this << packet << saddr << daddr << sport << dport << oif);
...
void
TcpL4Protocol::SendPacket(Ptr<Packet> packet, const TcpHeader &outgoing, Ipv6Address saddr, Ipv6Address daddr, Ptr<NetDevice> oif)
{
  NS_LOG_LOGIC ("TcpL4Protocol " << this
      << " sending seq " << outgoing.GetSequenceNumber ()
      << " ack " << outgoing.GetAckNumber ()
